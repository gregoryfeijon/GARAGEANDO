...
public abstract class Builder<T> {
	...
	protected T clone(T t) {
		...
		try {
			...
			List<Method> getters = methods.stream().filter(
					m -> m.getName().toLowerCase().startsWith("get") || m.getName().toLowerCase().startsWith("is"))
					.collect(Collectors.toList());
			List<Method> setters = methods.stream().filter(m -> m.getName().toLowerCase().startsWith("set"))
					.collect(Collectors.toList());
